...
class AirlineSentiment:
  ...
  def train_kfold(self):
    ...
    X = self.text_to_word_embeddings(self.df['text'].values, self.max_len)
  ...
  def build_model(self, max_len):
    ...
    model.add(Embedding(input_dim=self.vocab_size, output_dim=EMBED_DIMS, input_length=max_len, weights=[self.embed_matrix], trainable=False))
  ...
  def create_word_embeddings_dict(self):
    ...
    emb_dict = {}
    glove = open(self.w2v_file, 'r', encoding = "utf-8")
    for line in glove:
        values = line.split()
        word = values[0]
  ...
  def text_to_word_embeddings(self, texts, max_len):
